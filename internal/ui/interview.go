// Package ui renders claunch's guided launch — a small huh interview that picks
// the model and the session disposition before claunch execs claude.
package ui
...
// terminal.
func Interview(p config.Profile) (Choice, error) {
	model := p.Model
	session := "new"
...
func sessionMode(s string) launch.SessionMode {
	switch s {
	case "resume":
